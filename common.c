...
int Utility_GetGprsServerIp(char* _str, int* len) {
	...
	*len = Utility_Strlen(configpara.serverip);
	...
	if (*len > REMOTE_GPRS_SERVER_IP_ADDR_LEN) {
		Console_WriteErrorStringln(
			"get gprs server ip faied,len is out of limit");
		return _ret;
	}
...
int Utility_GetGprsServerPort(char* _str, int* len) {
	...
	*len = Utility_Strlen(configpara.serverport);
	...
	if (*len > REMOTE_GRPS_SERVER_PORT_ADDR_LEN) {
		Console_WriteErrorStringln(
			"get gprs server port faied,len is out of limit");
		return _ret;
	}
...
int Utility_GetGprsServerApn(char* _str, int* len) {
	...
	*len = Utility_Strlen(configpara.serverapn);
	...
	if (*len > REMOTE_GPRS_SERVER_APN_ADDR_LEN) {
		Console_WriteErrorStringln(
			"get gprs server apn faied,len is out of limit");
		return _ret;
	}
...
void Utility_CalculateNextReportTimeBytes(
	char* dest)  //根据当前设置,计算出下一次报告时间
{		     //报告时间 5分钟
	for (int i = 0; i < 5; ++i) {
	...
	char temp[ 2 ];
	int  mode = 0;
	if (Store_ReadReportTimeMode(temp)
	    < 0) {  //没读出模式 就当5分钟一存好了.
		mode = 1;
	}
	else
...
void Utility_CalculateNextCameraGoTimes(
	char* dest)  //根据当前设置,计算出下一次摄像时间
{		     //报告时间 5分钟
	for (int i = 0; i < 5; ++i) {
		dest[ i ] = g_rtc_nowTime[ i ];
	}
	...
	char temp[ 2 ];
	int  mode = 0;
	if (Store_ReadCameraTimeMode(temp)
	    < 0) {  //没读出模式 就当5分钟一存好了.
		mode = 1;
	}
	else
...
void Utility_CalculateNextSaveTimeBytes(
	char* dest)  //根据当前设置,计算出下一次保存时间
{		     //保存时间 5分钟
	for (int i = 0; i < 5; ++i) {
		dest[ i ] = g_rtc_nowTime[ i ];
	}
...
void Utility_CalculateNextCheckTimeBytes(
	char* dest)  //根据当前时间,计算下一次检查时间
{		     //检查时间 5分钟
	for (int i = 0; i < 5; ++i) {
		dest[ i ] = g_rtc_nowTime[ i ];
	}
...
int Utility_Is_A_CheckTime(char* _time) {
	...
	timerinterval = _BCDtoHEX(timerinterval);
	...
	case 7:
		...
		break;
	case 8:
		if (_time[ 3 ] % 4 == 0 && _time[ 4 ] == 0)
			return 1;
		break;
	case 9:
		if ((_time[ 3 ] % 12 == 0 || _time[ 3 ] == 0)
		    && _time[ 4 ] == 0)
			return 1;
		break;
	case 10:
		...
		break;
	case 11:
		if ((_time[ 2 ] - 1) % 2 == 0 && _time[ 3 ] == 0
		    && _time[ 4 ] == 0)
			return 1;
		break;
	case 12:
		if ((_time[ 2 ] - 1) % 3 == 0 && _time[ 3 ] == 0
		    && _time[ 4 ] == 0)
			return 1;
		break;
	case 13:
		if ((_time[ 2 ] - 1) % 5 == 0 && _time[ 3 ] == 0
		    && _time[ 4 ] == 0)
			return 1;
		break;
	case 14:
		if ((_time[ 2 ] - 1) % 10 == 0 && _time[ 3 ] == 0
		    && _time[ 4 ] == 0)
			return 1;
		break;
	case 15:
		if ((_time[ 2 ] - 1) % 15 == 0 && _time[ 3 ] == 0
		    && _time[ 4 ] == 0)
			return 1;
		break;
	case 16:
		if (_time[ 2 ] == 1 && _time[ 3 ] == 0 && _time[ 4 ] == 0)
			return 1;
		break;
	default:
		break;
...
int Utility_Is_A_SaveTime(char* _time) {
	char timerinterval;
	char endtime[ 6 ] = { 0, 0, 0, 0, 0, 0 };
	Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_STORE_INTERVAL,
				&timerinterval,
...
int Utility_Is_A_CameraTime(char* _time) {
	char _temp[ 2 ];
	int  _mode = 0;
	if (Store_ReadCameraTimeMode(_temp)
	    < 0) {  //没读出模式 就当5分钟一存好了.
		_mode = 0;
	}
	else
