...
ETL::ETL(unsigned long long physical_capacity)
...
void ETL::SetInfoPage(InfoPage infopage) {
	...
	this->RomWriteBytes(this->physical_capacity_ - infopage_size,
			    ( char* )&infopage, infopage_size);
...
InfoPage ETL::GetInfoPage() {
	InfoPage infopage;
	size_t	 infopage_size = sizeof(InfoPage);
	this->RomReadBytes(this->physical_capacity_ - infopage_size,
			   ( char* )&infopage, infopage_size);
	this->info_page_ = infopage;
...
void ETL::InitialPhysicalPages() {
	char* data =
		( char* )calloc(this->info_page_.logic_page_size, sizeof(char));
	...
	for (int physical_page_num = 0;
	     physical_page_num < this->info_page_.total_page_count;
	     ++physical_page_num) {
		datapage.logic_page_num = physical_page_num;
		this->lpn_to_ppn_[ datapage.logic_page_num ] =
			physical_page_num;
...
bool ETL::WriteDataPage(int physical_page_num, DataPage* datapage) {
	...
	memcpy(buff + offest, ( char* )&datapage->erase_cycle,
	       sizeof(datapage->erase_cycle));
	...
	memcpy(buff + offest, ( char* )&datapage->logic_page_num,
	...
	memcpy(buff + offest, ( char* )&datapage->check_sum,
	       sizeof(datapage->check_sum));
	offest += sizeof(datapage->check_sum);

	memcpy(buff + offest, ( char* )&datapage->data,
	       this->info_page_.logic_page_size);

	this->RomWriteBytes(datapage_size * physical_page_num, buff,
			    datapage_size);
...
bool ETL::ReadDataPage(int physical_page_num, DataPage* datapage) {
	...
	this->RomReadBytes(datapage_size * physical_page_num, buff,
			   datapage_size);

	memcpy(( char* )&datapage->erase_cycle, buff + offest,
	       sizeof(datapage->erase_cycle));
	offest += sizeof(datapage->erase_cycle);
	...
	memcpy(( char* )&datapage->logic_page_num, buff + offest,
	       sizeof(datapage->logic_page_num));
	offest += sizeof(datapage->logic_page_num);
	...
	memcpy(( char* )&datapage->check_sum, buff + offest,
	       sizeof(datapage->check_sum));
	offest += sizeof(datapage->check_sum);

	memcpy(( char* )&datapage->data, buff + offest,
	       this->info_page_.logic_page_size);
...
unsigned int ETL::GetDataPageSize() {
	return sizeof(DataPage::erase_cycle)
	       + sizeof(DataPage::effective_erase_cycle)
	       + sizeof(DataPage::logic_page_num) + sizeof(DataPage::hot)
	       + sizeof(DataPage::check_sum) + this->info_page_.logic_page_size;
}
