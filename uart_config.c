...
void Hydrology_ProcessBLEReceieve(){
	if (BLE_ISFINDBT()) {
		char	 buffer[ 300 ];
		memset(buffer, 0, 300);
		int	 count	  = 0;
		...
		if (ptDevBle->isspp()) {
			ptDevBle->getMsgNoWait(buffer, &count);
		}

		if (count != 0) {
			TraceHexMsg(buffer, count);
			isUARTConfig = 1;
			if(hydrologyProcessReceieve(buffer, count) != 0){
				ptDevBle->sendMsg("config failed",sizeof("config failed"));
			}else{
				ptDevBle->sendMsg("config success",sizeof("config success"));
			}
		}
	}
}
